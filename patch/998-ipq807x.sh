...
sed -i '$a https://downloads.openwrt.org/snapshots/targets/qualcommax/ipq807x/kmods/6.12.87-1-467536ea7caaffdcf2daf461fb4183a9/packages.adb' /etc/apk/repositories.d/distfeeds.list
...
if uci -q get wireless.default_radio2 >/dev/null; then
    uci set wireless.default_radio2.ssid=WiFi-$(cat /sys/class/ieee80211/phy0/macaddress|awk -F ":" '{print $5""$6 }' | tr 'a-z' 'A-Z')
fi
