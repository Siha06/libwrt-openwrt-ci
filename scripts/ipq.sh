...
if grep -Eq "luci-app-kwrt=y" .config; then
    mv $GITHUB_WORKSPACE/vm/libwrt-istore vermagic
...
sed -i "s/%V/25.12.4/g" package/base-files/files/usr/lib/os-release
sed -i "s/%V/25.12.4/g" package/base-files/files/etc/openwrt_release
#添加编译日期标识
